<template>
  <div>
    ...
    <vs-row
      v-if="$helper.isAllowed('browse_post_theme_configuration') && groupList.length > 0"
    >
      <vs-col vs-lg="12">
        <vs-card>
          <vs-tabs :color="adminPanelHeaderFontColor">
            ...
            >
              <vs-row
                ...
                v-for="(config, index) in filterConfigurations('postTheme')"
                :key="index"
              >
                <badaso-text
  ...
  computed: {
    groupList: {
      get() {
        return [
          {
            label: 'Post Theme Configuration',
            value: 'postTheme'
          }
        ]
      },
