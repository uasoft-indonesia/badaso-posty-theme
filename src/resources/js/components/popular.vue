<template>
  ...
  <div v-else>
    <vs-row vs-justify="center" vs-align="flex-end" vs-type="flex" v-if="posts && posts.length > 0">
      <vs-col>
        <vs-card class="simple-theme__popular--card mb-0">
          <div slot="header" @click="$to('popular')">
            <vs-icon icon="thumb_up_alt" color="white"></vs-icon>
            <span @click="$to('popular')" class="simple-theme__popular--title ml-8">Berita Terpopuler</span>
          </div>
          <div class="simple-theme__popular--card-content-container" v-for="(post, index) in posts" :key="index">
            <vs-row vs-justify="center" vs-align="center">
              <vs-col vs-w="10">
                <span class="pointer" @click="$to('post', post.slug)">
                  <vue-clamp :max-lines="3" class="simple-theme__popular--content-title">
                    {{ post.title }}
                  </vue-clamp>
                </span>
              </vs-col>
