      ...
      <vs-col>
        <vs-card class="post-theme__newest--card mb-0">
          <div slot="header" @click="$to('newest')">
            ...
            <span @click="$to('newest')" class="post-theme__newest--title ml-8">Berita Terbaru</span>
          </div>
          <div class="post-theme__newest--card-content-container" v-for="(post, index) in posts" :key="index">
            <vs-row vs-type="flex" vs-justify="center" vs-align="center">
              ...
              <vue-clamp v-if="$isMobile" :max-lines="5" class="post-theme__newest--card-description">
                {{ post.summary }}
                <template slot="after">
                  <span @click="$to('post', post.slug)" class="post-theme__newest--read-more">Baca Selengkapnya</span>
                </template>
              </vue-clamp>
