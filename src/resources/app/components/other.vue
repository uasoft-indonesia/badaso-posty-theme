  ...
  <div v-else>
    <vs-row vs-justify="center" vs-align="flex-end" vs-type="flex" v-if="posts && posts.length > 0">
      <vs-col vs-w="6" vs-justify="flex-start" vs-align="center" vs-type="flex" class="mb-8">
        <Link :href="route('badaso.post-theme.search')" class="post-theme__category--title">Berita Lainnya</Link>
      </vs-col>
          ...
          <div class="post-theme__other--card-content-container" v-for="(post, index) in posts" :key="index">
            <vs-row vs-type="flex" vs-justify="center" vs-align="flex-start">
              <vs-col vs-xs="12" vs-sm="12" vs-lg="4">
                <Link style="display: block;" :href="route('badaso.post-theme.post', post.slug)">
                  <img height="240" :src="post.thumbnail" :class="{pointer: true, 'mb-20': $isMobile()}">
                </Link>
              </vs-col>
              <vs-col vs-xs="12" vs-sm="12" vs-lg="8" class="pl-24">
                <Link :href="route('badaso.post-theme.post', post.slug)" class="post-theme__other--content-title">
                  {{ post.title }}
                </Link>
