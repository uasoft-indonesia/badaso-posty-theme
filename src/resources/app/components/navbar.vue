  ...
  <nav>
    <vs-row class="post-theme__navbar" vs-justify="center" vs-align="center" vs-type="flex">
      ...
      <vs-col vs-xs="8" vs-sm="8" vs-lg="7" class="post-theme__navbar--title" vs-justify="start" vs-align="center" vs-type="flex">
        <Link :href="route('badaso.post-theme.home')" :class="{ 'ml-16': $isMobile(), 'ml-32': !$isMobile() }">{{ getTitle }}</Link>
      </vs-col>
    ...
    <vs-row class="mb-30 post-theme__menu" vs-justify="start" vs-align="center" vs-type="flex" :class="{ open: open }">
      <vs-col vs-xs="4" vs-sm="4" vs-lg="2" class="post-theme__menu--item"><Link :href="route('badaso.post-theme.home')">HOME</Link></vs-col>
      <vs-col vs-xs="4" vs-sm="4" vs-lg="2" class="post-theme__menu--item" v-for="(category, index) in getCategories" :key="index"><Link :href="route('badaso.post-theme.category', category.slug)">{{ category.title }}</Link></vs-col>
    </vs-row>
  ...
  methods: {
    getSearchedPosts() {
      ...
      if (this.search !== null || this.search !== "") {
        this.$inertia.visit(this.route('badaso.post-theme.search', this.search))
      }
