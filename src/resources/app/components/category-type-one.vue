    ...
    <template v-if="$isMobile()">
      <vs-row vs-justify="center" vs-align="flex-end" vs-type="flex" v-if="posts && posts.length > 0">
        <vs-col vs-w="6" vs-justify="flex-start" vs-align="center" vs-type="flex" class="mb-4">
          <Link :href="route('badaso.post-theme.category', posts[0].category.slug)" class="post-theme__category--title">{{ posts[0].category.title }}</Link>
        </vs-col>
        <vs-col vs-w="6" vs-justify="flex-end" vs-align="center" vs-type="flex" class="mb-4">
          <Link :href="route('badaso.post-theme.category', posts[0].category.slug)" class="d-flex justify-content-center align-items-center text-black"><span class="post-theme__category--see-all">LIHAT SEMUA</span> <vs-icon icon="chevron_right" size="12px"></vs-icon></Link>
        </vs-col>
        <vs-col>
          <vs-card class="post-theme__category--card mb-20">
            <div slot="media" class="post-theme__category--card-header-container">
              ...
              <div class="post-theme__category--card-header">
                <vs-row>
                  ...
                  <vs-col>
                    <Link :href="route('badaso.post-theme.post', posts[0].slug)">
                      <h3 class="post-theme__category--card-header-text">{{ posts[0].title }}</h3>
                    </Link>
                  </vs-col>
                </vs-row>
              </div>
            ...
            <div class="post-theme__category--card-content-container">
              <div class="post-theme__category--card-description line-clamp-3" v-html="posts[0].content"></div>
              <Link :href="route('badaso.post-theme.post', posts[0].slug)" class="post-theme__category--read-more">Baca Selengkapnya</Link>
            ...
            <vs-card :class="{ 'post-theme__category--card': true, 'mb-20': index !== secondaryPost.length - 1, 'mb-0': index === secondaryPost.length - 1}" v-for="(post, index) in secondaryPost" :key="index" :style="'background-image: url(' + post.thumbnail + ')'">
              <div slot="media" class="post-theme__category--card-header-container-secondary">
                <div class="post-theme__category--card-header-secondary">
                  <vs-row>
                    <vs-col class="mb-8">
                      <Link :href="route('badaso.post-theme.category', post.category.slug)">
                    ...
                    <vs-col>
                      <Link :href="route('badaso.post-theme.post', post.slug)">
                        <vue-clamp tag="h3" :max-lines="2" class="post-theme__category--card-header-text">
                          {{ post.title }}
                        </vue-clamp>
                      </Link>
                    </vs-col>
            ...
            <vs-col vs-w="8" class="pr-30">
              <vs-card class="post-theme__category--card mb-0">
                <div slot="media" class="post-theme__category--card-header-container">
                  ...
                  <div class="post-theme__category--card-header z-20">
                    <vs-row>
                      <vs-col class="mb-8">
                        <Link :href="route('badaso.post-theme.category', posts[0].category.slug)">
                          <vs-chip color="#2E99A5" class="post-theme__category--card-header-chip">{{ posts[0].category.title }}</vs-chip>
                ...
                <div class="post-theme__category--card-content-container">
                  <div class="post-theme__category--card-description line-clamp-3 mb-4" v-html="posts[0].content"></div>
                  <Link :href="route('badaso.post-theme.post', posts[0].slug)" class="post-theme__category--read-more">Baca Selengkapnya</Link>
                  <vs-row vs-type="grid" class="h-100 place-content-start mt-16">
                    <vs-col vs-w="12" vs-type="grid" vs-justify="center" vs-align="center" class="grid-column mb-8">
              ...
              <template v-if="posts && posts.length > 1">
                <vs-row vs-type="flex" vs-justify="center" vs-align="center">
                  <vs-col v-for="(post, index) in secondaryPost" :key="index" class="relative">
                    <vs-card class="post-theme__category--card overlay__container" :style="'background-image: url(' + post.thumbnail + ')'">
                      <Link slot="media" class="post-theme__category--card-header-container-secondary p-16 pointer" :href="route('badaso.post-theme.post', post.slug)">
                        <div class="post-theme__category--card-header-secondary z-20">
                          <vs-row>
                            <vs-col class="mb-8">
                              <Link :href="route('badaso.post-theme.category', post.category.slug)">
                                <vs-chip color="#2E99A5" class="post-theme__category--card-header-chip">{{ post.category.title }}</vs-chip>
